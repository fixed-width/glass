...
#
# Four jobs need it — the mutation shards, the weekly sweep, `check` and `coverage` — because each
# runs `cargo test --workspace` or mutates a crate whose tests start a real display. Inlined four
# times, one copy had already drifted to describe a package list it no longer installed.
#
# The tests are not `#[ignore]`d: the mutation gate runs nextest, which skips ignored tests, so
# ignoring them would grade every mutant as caught.
set -euo pipefail
...
# Into the location glass's own discovery looks in. `pipefail` above is load-bearing: without it
# curl's status is discarded and a 404 is caught only if tar happens to choke on it.
