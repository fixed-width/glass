...
if ! command -v dbus-daemon >/dev/null 2>&1 \
   ...
   || ! python3 -c 'import cairo, gi; gi.require_foreign("cairo"); gi.require_version("Gtk", "4.0")' >/dev/null 2>&1; then
    ...
    echo "           gir1.2-gtk-3.0 gir1.2-gtk-4.0 python3-gi python3-cairo python3-gi-cairo xvfb dbus-bin). Skipping."
    exit 0
fi
