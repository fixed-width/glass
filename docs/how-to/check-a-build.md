...
- **A target app.** A real run drives a stock app installed on the host — install one of the
  candidates for the backend you are checking, listed under
  [Target apps](../reference/smoke.md#target-apps).
...
- **On `--backend wayland`, a discoverable sway ≥ 1.12.** The backend runs the app inside a private
  headless sway, so without one there is no compositor to launch into and check 1 (`start`) fails.
  [Set up Linux](setup-linux.md#wayland-sway) covers where glass looks for it.

None of these is needed for `--dry-run` — see below.
...
- `--app <name>` — drive a specific candidate instead of the first one probing finds. It selects
  among [the backend's candidates](../reference/smoke.md#target-apps); it does not make an app
  appear. A real run naming one you don't have fails at check 1; add `--dry-run` and the plan tells
  you up front instead.
...
4. If check 1 (`start`) failed on `--backend wayland`, read `doctor`'s `wayland` section: without a
   discoverable sway there is no compositor to launch into, and no choice of `--app` will help.
5. If none of those applies, install another candidate and retry with `--app <name>`, to see
...
6. Re-run with `--report <path>` and attach the JSON when
