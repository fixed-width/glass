...
/// glass#391: the report resolves the launcher through the crate that spawns it, so it honours
/// `GLASS_ATSPI_LAUNCHER` exactly as a launch does. Nothing else in the workspace can see that
/// delegation — hardcode `false` into `accessibility_launcher_present` and every other test still
/// passes.
///
/// Mutates the process environment, so it is `#[ignore]`d and runs only under the single-threaded
/// harness — hence the `unsafe_code` opt-out.
#[test]
#[ignore = "mutates the process environment; run via scripts/test-a11y.sh (--test-threads=1)"]
#[allow(unsafe_code)]
fn the_a11y_report_honours_the_launcher_override() {
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            // SAFETY: test-a11y.sh runs this suite with `--test-threads=1`, so no other thread is
            // reading the environment while it is mutated.
    ...
    // SAFETY: as above — single-threaded harness.
