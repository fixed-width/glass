...
fn starts_yields_addresses_and_reaps() {
    ...
    assert!(
        bus.session_bus_address().starts_with("unix:"),
        "session addr: {}",
        bus.session_bus_address()
    );
    assert!(
        bus.a11y_bus_address().starts_with("unix:"),
        "a11y addr: {}",
        bus.a11y_bus_address()
    );
...
fn a11y_socket_is_private_not_host_runtime_dir() {
    ...
    assert!(
        !addr.contains("/run/user/"),
        "a11y socket leaked into the host runtime dir: {addr}"
    );
    assert!(
        addr.contains("/at-spi/"),
        "expected an at-spi socket path: {addr}"
    );
}
